...
router.get('/', function(req, res, next) {
	if (req.query.state) {
		...
		client.execute(query, [req.query.state], function(err, results) {		// [] to put search params. Empty stands for none, i.e. grab everything
			if (err) {
		...
		client.execute(query, [], function(err, results) {		// [] to put search params. Empty stands for none, i.e. grab everything
			if (err) {
				res.status(404).send({ msg: err });
			} else {
				res.render('doctors', {
...
router.get('/details/:id', function(req, res, next) {
	...
	client.execute(query, [req.params.id], function(err, result) {		// [] to put search params
		if (err) {
			res.status(404).send({ msg: err });
		} else {
			res.render('details', {
...
router.get('/category/:name', function(req, res, next) {
	...
	client.execute(query, [req.params.name], function(err, results) {		// [] to put search params
		if (err) {
			res.status(404).send({ msg: err });
		} else {
			res.render('doctors', {
